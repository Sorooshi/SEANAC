...
if __name__ == '__main__':
    ...
    if run == 1:
        ...
        with open(os.path.join('SANC_computation', name + ".pickle"), 'rb') as fp:
            DATA = pickle.load(fp)
                    ...
                    if str(setting) == setting_:
                        ...
                        for repeat, matrices in repeats.items():
                            ...
                            if name.split('(')[0][-1] == 'C':
                                ...
                                Y, _, Yz, _, Yrng, _, = preprocess_Y(Yin=Y_oneHot, data_type='C')  # Y_oneHot
        ...
        if setting_ != 'all':
            ...
            with open(os.path.join('SANC_computation', "out_ms_" + name + "-" + pp + "-" + setting_ + ".pickle"), 'wb') as fp:
                pickle.dump(out_ms, fp)

        if setting_ == 'all':

            with open(os.path.join('SANC_computation', "out_ms_" + name + "-" + pp + "-" + ".pickle"), 'wb') as fp:
                pickle.dump(out_ms, fp)
        ...
        for setting, results in out_ms.items():
            ...
            for repeat, result in results.items():
                gt, _ = flat_cluster_results(result)
                lp, _ = flat_ground_truth(DATA[setting][repeat]['GT'])
    ...
    if run == 0:
        ...
        if with_noise == 1:
            name = name + '-N'
        with open(os.path.join('SANC_computation', "out_ms_" + name + "-" + pp + ".pickle"), 'rb') as fp:
